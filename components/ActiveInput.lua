...
return Solyd.wrapComponent("ActiveInput", function(props)
    --print("Test")
    ...
    local displayText = inputState.value:sub(inputState.viewPort, inputState.viewPort + props.width - 1)
    local displayColor = props.color
    if not inputState.value or inputState.value == "" then
        displayText = props.placeholder
        displayColor = props.placeholderColor
    end
    ...
    },
    {
        ...
        aabb = useBoundingBox((props.x*2)-1, (props.y*3)-2, (props.width)*2, (props.height)*3,
            function() -- onClick
                --inputState.active = true
                inputState.viewPort = inputState.viewPort or math.max(1, inputState.cursorPos - props.width + 1)
                inputState.cursorX = inputState.cursorX or (props.x + inputState.cursorPos - inputState.viewPort)
                setInputState(inputState)
            end,
            ...
            function(key, held)
                ...
                elseif key == keys.enter then
                    ...
                    if props.onEnter then
                        if props.type == "number" and inputState.value ~= nil then
                            props.onEnter(tonumber(inputState.value))
                        elseif props.type == "colorpicker" and inputState.value ~= nil then
                            -- Convert hex to number
                            local hex = inputState.value
                            hex = hex:gsub("#", "")
                            hex = hex:gsub("x", "")
                            props.onEnter(tonumber(hex, 16))
                        else
                            props.onEnter(inputState.value)
                        end
                    end
                    setInputState(inputState)
